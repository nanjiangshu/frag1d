...
int main(int argc, char** argv)/*{{{*/
{
    ...
    while(i < argc)/*{{{*/
    {
        if(argv[i][0] == '-' && !isNonOptionArg) //options
        {
            ...
            {
                if( ( i = option_parser_filename_old(argc, argv, i, seqmappath)) == -1)
                    return -1;
            }
            else if (strcmp(argv[i], "-list") == 0)
            {
                if( ( i = option_parser_filename_old(argc, argv, i, pssmFileList)) == -1)
                    return -1;
            }
            ...
            {
                if( ( i = option_parser_filename_old(argc, argv, i, maxaccfile)) == -1)
                    return -1;
            }
            else if (strcmp(argv[i], "--accscale") == 0)
            {
                if( ( i = option_parser_filename_old(argc, argv, i, accscalefile)) == -1)
                    return -1;
            }
            else if (strcmp(argv[i], "--ext") == 0)
            {
                if( ( i = option_parser_filename_old(argc, argv, i, modmfileext)) == -1)
                    return -1;
            }
