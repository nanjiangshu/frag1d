...
int main(int argc, char** argv)/*{{{*/
{
    ...
    Array1D <char>  outpath_1darray(MAX_PATH+1);
    Array1D <char>  qijfileext_1darray(MAX_PATH+1);
    Array1D <char>  bkfreqfile_1darray(MAX_PATH+1);
    Array1D <char>  maxaccfile_1darray(MAX_PATH+1);
    Array1D <char>  accscalefile_1darray(MAX_PATH+1);
    Array1D <char>  submatfile_1darray(MAX_PATH+1);
    Array1D <char>  pssmFileList_1darray(MAX_PATH+1);
    Array1D <char>  seqmappath_1darray(MAX_PATH+1);
    Array1D <char>  datadir_1darray(MAX_PATH+1);

    char *outpath      = outpath_1darray.array1D;
    char *qijfileext   = qijfileext_1darray.array1D; // default qijfile extension is Qij
    char *bkfreqfile   = bkfreqfile_1darray.array1D;
    char *maxaccfile   = maxaccfile_1darray.array1D; // file list the maximal water accessibility for each amino acid
    char *accscalefile = accscalefile_1darray.array1D; // file list the scale for mapping water accessibility to 0~9
    char *submatfile   = submatfile_1darray.array1D;
    char *pssmFileList = pssmFileList_1darray.array1D; /* added 2008-07-06                                              */ 
    char *seqmappath   = seqmappath_1darray.array1D;

    char *datadir = datadir_1darray.array1D;

    /*setting default values for file names*/
    strcpy(bkfreqfile    , "");
    strcpy(maxaccfile    , "");
    strcpy(accscalefile  , "");
    strcpy(submatfile    , "");
    strcpy(pssmFileList  , "");
    strcpy(qijfileext, "Qij");
    strcpy(outpath, "./");
    ...
    while(i < argc){/*{{{*/ 
        if(argv[i][0] == '-' && !isNonOptionArg) //options
        {
            if(strcmp(argv[i], "-h") == 0|| strcmp(argv[i], "--help") == 0)
            {
                ...
                //next item will be a non-option argument
                isNonOptionArg = true;
                i ++;
                continue;
            } else {
